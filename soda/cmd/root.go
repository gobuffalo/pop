...
var RootCmd = &cobra.Command{
	...
	PersistentPreRun: func(c *cobra.Command, args []string) {
		log.DefaultLogger.WithField("version", Version).Info("Starting")
...
func Execute() {
	if err := RootCmd.Execute(); err != nil {
		log.DefaultLogger.WithField("error", err).Fatal("Failed")
	}
}
...
func getConn() *pop.Connection {
	...
	if conn == nil {
		log.DefaultLogger.WithField("environment", env).Fatal("There is no connection for this environment")
	}
	return conn
}
