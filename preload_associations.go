...
func (mmi *ModelMetaInfo) preloadFields(fields ...string) ([]*reflectx.FieldInfo, error) {
	...
	for _, f := range fields {
		...
		if strings.Contains(f, ".") {
			key := f[:strings.Index(f, ".")]
			if _, ok := mmi.nestedFields[key]; !ok {
				mmi.nestedFields[key] = make([]string, 0)
			}
			mmi.nestedFields[key] = append(mmi.nestedFields[key], f[strings.Index(f, ".")+1:])
			f = f[:strings.Index(f, ".")]
		}
