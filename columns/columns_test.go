...
func Test_Columns_Basics(t *testing.T) {
	...
	for _, f := range []any{foo{}, &foo{}} {
		...
		r.Equal(
			c.Cols["first_name"],
			&columns.Column{Name: "first_name", Writeable: false, Readable: true, SelectSQL: "first_name as f"},
		)
		r.Equal(
			c.Cols["LastName"],
			&columns.Column{Name: "LastName", Writeable: true, Readable: true, SelectSQL: "foo.LastName"},
		)
		r.Equal(c.Cols["read"], &columns.Column{Name: "read", Writeable: false, Readable: true, SelectSQL: "foo.read"})
		r.Equal(
			c.Cols["write"],
			&columns.Column{Name: "write", Writeable: true, Readable: false, SelectSQL: "foo.write"},
		)
	}
}
...
func Test_Columns_IDField_Readonly(t *testing.T) {
	...
	r.Equal(1, len(c.Cols), "%+v", c)
	r.Equal(
		&columns.Column{Name: "id", Writeable: false, Readable: true, SelectSQL: "with_id_readonly.id"},
		c.Cols["id"],
	)
}
...
func Test_Columns_ID_Field_Not_ID(t *testing.T) {
	...
	r.Equal(1, len(c.Cols), "%+v", c)
	r.Equal(
		&columns.Column{Name: "notid", Writeable: false, Readable: true, SelectSQL: "non_standard_id.notid"},
		c.Cols["notid"],
	)
}
