...
func genericUpdate(s store, model *Model, cols columns.Columns) error {
	...
	log.DefaultLogger.WithField("query", stmt).Debug("Updating record")
...
func genericExec(s store, stmt string) error {
	log.DefaultLogger.WithField("query", stmt).Debug("Executing query")
	_, err := s.Exec(stmt)
	if err != nil {
		return errors.WithStack(err)
	}
...
func genericSelectOne(s store, model *Model, query Query) error {
	...
	log.DefaultLogger.WithField("query", sql).WithField("args", args).Debug("Retrieving record")
...
func genericSelectMany(s store, models *Model, query Query) error {
	...
	log.DefaultLogger.WithField("query", sql).WithField("args", args).Debug("Retrieving records")
...
func genericLoadSchema(deets *ConnectionDetails, migrationURL string, r io.Reader) error {
	...
	if len(contents) == 0 {
		log.DefaultLogger.WithField("database", deets.Database).Debug("schema is empty, skipping")
		return nil
	}
	...
	log.DefaultLogger.WithField("database", deets.Database).Info("loaded schema")
	return nil
}
